...
from wing_loss import wing_loss
...
def parse_arguments():
    ...
    parser.add_argument("--previous_name", "-pn", help="Name of your trained model (which was saved using this name)",
                        default=None)
    parser.add_argument("--patience", "-pat", help="Patience parameter of scheduler ReduceLROnPlateau", default=1, type=int)
    parser.add_argument("--factor", "-fac", help="Factor parameter of scheduler ReduceLROnPlateau", default=0.1, type=float)
    parser.add_argument("--best-val-loss", "-bestvl", help="Best loss on validation sample (use with pretrained model)",
                        default=None, type=float)
    parser.add_argument("--loss", "-loss", help="Loss function", default="mse_loss")
...
def main(args):
    ...
    model.to(device)
    if(args.previous_name):
        with open(f"{args.previous_name}_best.pth", "rb") as fp:
            best_state_dict = torch.load(fp, map_location="cpu")
            model.load_state_dict(best_state_dict)

    optimizer = optim.AdamW(model.parameters(), lr=args.learning_rate, amsgrad=True)
    lr_scheduler = ReduceLROnPlateau(optimizer, factor=args.factor,
                                     patience=args.patience, min_lr=args.learning_rate, verbose=True)
    loss_fn = fnn.mse_loss
    if args.loss == "mse_loss":
        loss_fn = fnn.mse_loss
    if args.loss == "wing_loss":
        loss_fn = wing_loss
    if args.loss == "huber_loss":
        loss_fn = fnn.smooth_l1_loss

    # 2. train & validate
    print("Ready for training...")
    if args.best_val_loss:
